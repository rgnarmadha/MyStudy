...
public class OpenSsoLoginServletTest {
  OpenSsoLoginServlet servlet;
  TrustedTokenServiceImpl trustedTokenService;

  @Mock
  OpenSsoAuthenticationHandler handler;
  ...
  public void setUp() throws Exception {
    ...
    servlet = new OpenSsoLoginServlet(handler, trustedTokenService);
  ...
  public void redirectWithoutTarget() throws Exception {
    ...
    when(request.getAttribute(OpenSsoAuthenticationHandler.AUTHN_INFO)).thenReturn(null);
    servlet.service(request, response);
  ...
  public void authAndAddToken() throws Exception {
    ...
    when(request.getAttribute(OpenSsoAuthenticationHandler.AUTHN_INFO)).thenReturn(authnInfo);
    when(request.getRequestURI()).thenReturn("someURI");
    when(request.getAttribute(Authenticator.LOGIN_RESOURCE)).thenReturn("greatplace");
    servlet.service(request, response);
