...
/**
 * Tests {@link LDAPConnection} liveness by executing a search filter and
 * verifying that the result contains at least one entry. This strategy is
 * appropriate when {@link LDAPConnection#isConnectionAlive()} is not reliable,
 * e.g. when running against a non-OpenLDAP service provider.
 *
 * <p>
 * This particular implementation makes an effort to generate filters which will
 * be easily traceable in LDAP logs. Generally, filters take the form
 * <code>(|(objectclass=*)({attr}={unique-string}))</code>. The search itself is
 * limited by {@link LDAPConnection#SCOPE_BASE}, where "BASE" is specified by
 * <code>baseDn</code>. Often, <code>baseDn</code> corresponds to the DN of the
 * "system user" as whom {@link JLDAPDirectoryProvider} binds when running in
 * "autoBind" mode.
 * </p>
 *
...
public class SearchExecutingLdapConnectionLivenessValidator implements
  ...
  /**
   * The searchFilter string with a placeholder for a unique key for a
   * particular execution. Treated as a {@link MessageFormat} pattern. Depends
   * on {@link #searchAttributeName} having already been initialized to some
   * meaningful value.
   */
  ...
  /**
   * The DN to be searched by {@link #searchFilter}. Searches are restricted to
   * search this DN only.
   */
  ...
  /**
   * Returns localhost's name as reported by {@link InetAddress#getLocalHost()
   * #toString()}. Factored into a method to enable override during testing.
   *
   * @return
  ...
  public boolean isConnectionAlive(LDAPConnection connectionToTest) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug("isConnectionAlive(): executing connection liveness search [base dn = " + baseDn
            + "][filter = " + formattedSearchFilter + "][return attrib = " + searchAttributeName
            + "]");
      }

      LDAPSearchResults searchResults = connectionToTest.search(baseDn, LDAPConnection.SCOPE_BASE,
          formattedSearchFilter, new String[] { searchAttributeName }, false, searchConstraints);

      if (log.isDebugEnabled()) {
        log.debug("isConnectionAlive(): executed search [base dn = " + baseDn + "][filter = "
            + formattedSearchFilter + "][return attrib = " + searchAttributeName + "]");
      }

      if (searchResults.hasMore()) {
        if (log.isDebugEnabled()) {
          log.debug("isConnectionAlive(): search contained results [base dn = " + baseDn
              + "][filter = " + formattedSearchFilter + "][return attrib = " + searchAttributeName
              + "]");
        }
        ...
        if (log.isDebugEnabled()) {
          log.debug("isConnectionAlive(): search [base dn = " + baseDn + "][filter = "
              + formattedSearchFilter + "][return attrib = " + searchAttributeName
              + "] had results, returning [" + isNonNullEntry + "]");
        }
        return isNonNullEntry;
      } else {
        if (log.isDebugEnabled()) {
          log.debug("isConnectionAlive(): search had no results [base dn = " + baseDn
              + "][filter = " + formattedSearchFilter + "][return attrib = " + searchAttributeName
              + "], returning false");
        }
        return false;
      ...
      if (log.isDebugEnabled()) {
        log.debug("isConnectionAlive(): liveness test failed [base dn = " + baseDn + "][filter = "
            + formattedSearchFilter + "][return attrib = " + searchAttributeName + "]", le);
      }
      return false;
    }
  ...
  protected String newUnformattedSearchFilter() {
    return new StringBuilder("(|(objectclass=*)(").append(searchAttributeName).append(
        "=validateProbe-").append(searchStamp).append("-{0}))").toString();
  }

  /**
   * Generates an executable search filter string by injecting the result of
   * {@link #generateUniqueSearchFilterTerm()} into the current
   * <code>searchFilter</code>. This term is usually treated as a portion of the
   * value to match against <code>searchAttributeName</code>
   *
   ...
   * identify an execution of that filter. By default concatenates a semi-unique
   * token ({@link #generateUniqueToken()} and the local host name, separated by
   * a dash ("-").
   *
  ...
  /**
   * Just returns the current system time in millis. This is factored into a
   * method so it can be overriden in testing (otherwise unique tokens are quite
   * difficult to verify).
   *
  ...
  /**
   * Assign the host name to be appended to (semi) invocation-unique search
   * terms. Falls back to {@link #DEFAULT_HOST_NAME} if argument is
   * <code>null</code>. If this setter is not ivoked, {@link #init()} will
