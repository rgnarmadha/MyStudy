...
public class ItemManager implements Dumpable, ItemStateListener {
    ...
    private SharedThreadDetector sharedThreadDetector;
    ...
    public synchronized NodeImpl getNode(NodeId id, NodeId parentId)
      ...
      try {
      if ( sharedThreadDetector == null ) {
        sharedThreadDetector = new SharedThreadDetector(session);
      }
      sharedThreadDetector.check(session);
      } catch ( Throwable t ) {
        t.printStackTrace();
      }
        if (parentId == null) {
