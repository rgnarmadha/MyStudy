...
import org.sakaiproject.nakamura.webdav.SlingItemFilter;
...
public class SlingWebDavServlet extends SimpleWebdavServlet {
  ...
  private static final long serialVersionUID = -4040977006826590912L;
    ...
    private SlingItemFilter slingItemFilter = new SlingItemFilter();
    
    private SlingLocatorMapper slingLocatorMapper = new SlingLocatorMapper();
    ...
    public DavLocatorFactory getLocatorFactory() {
        if (locatorFactory == null) {
            ...
            locatorFactory = new SlingLocatorFactory(workspace, slingLocatorMapper);
        }
    ...
    protected void activate(ComponentContext context)
        ...
        resourceConfig = new SlingResourceConfig(mimeTypeService,
            context.getProperties(), slingItemFilter);
    ...
    protected boolean execute(WebdavRequest request, WebdavResponse response,
        ...
        try {
            slingLocatorMapper.bindSession(getSessionProvider(),request,response,resource);
            return super.execute(request, response, method, resource);
        } finally {
            slingLocatorMapper.unbindSession();
        }
    }
}
